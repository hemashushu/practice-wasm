...
//             ins_branch::exec_block(vm_module, block_type, body)
//         }
//         Instruction::Loop { block_type, body } => {
//             ins_branch::exec_loop(vm_module, block_type, body)
//         }
//         Instruction::If {
//             block_type,
//             consequet_body,
...
//         Instruction::F64Store(memory_args) => ins_memory::f64_store(vm_module, memory_args),
        object::Instruction::Original(instruction) => {
            let result = match instruction {
                ...
                // 其他指令已经被替换成 Instruction::Control
                _=>{
                    unreachable!()
                }
            };
