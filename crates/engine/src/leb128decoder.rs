...
//
// 对于无符号数，需要从最高位开始补上零个或多个 0 以形成 uint32 或者 uint64。
//
// ## 有符号整数
//
// 对于有符号整数，拼接后的最高位是符号位，比如对于一个 int32 数：
//
// byte 0     byte 1
// ---------  ---------
// 1000 0001, 0111 1010
...
// byte 1   byte 0
//
// 最高位是 1，表示这个数是负数，需要从最高位开始补上零个或多个 1 以形成 int32 或者 int64
//
// 11 111001 00000001
// || -----/ \------/
// || byte 1  byte 0
    ...
    loop {
        remains = if let Some((byte, rest)) = remains.split_first() {
            ...
            if byte & CONTINUE_BIT == 0 {
                ...
                // 是的话还需要把高端所有位都补上 1 以形成 int64。
                if byte & SIGN_BIT != 0 {
                    // todo::
                    // 这里可以验证除索引 7、6 位之外，从高端开始第一个非 1 之前的
                    // 所有比特位都为 1。
...
mod test {
    ...
    use super::{decode_i64, decode_u64};
    ...
    fn test_decode_i64() {
        ...
        // 测试最后一个字节的符号位为 1 但其他高端位不为 1 的情况
        let d7: [u8; 10] = [
            0xf0,
            0xf1,
            0xf2,
