...
pub struct Module {
    ...
    ///
    /// 在文本格式里，除了可以在 type 段里定义的类型，导入函数也能产生新的类型，另外在定义函数时也能
    /// 产生新的类型。
    ...
    /// `函数-类型` 列表，（section id 3）
    /// 按顺序列出所有的**内部**函数所对应的类型，
    /// 函数的主体部分则位于代码列表。
    ///
    /// 注意，
    /// 一个模块的函数既包含导入函数，也包括内部定义的普通函数，
    /// `call` 等指令会跟着一个函数索引，该索引值是包括了导入函数以及
    /// 内部函数在内的，所有函数排列在一起之后的索引。
    /// 也就是说，第一个内部函数的 `函数索引值` 并非总是 0，
    /// 当一个模块有导入的函数时，`函数索引值` 从导入函数先开始计算，
    /// 所以第一个内部函数的 `函数索引值` 等于导入函数的数量。
    pub internal_function_to_type_index_list: Vec<u32>,
